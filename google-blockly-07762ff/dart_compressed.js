...
Blockly.Dart.init = function (a) {
    ...
    for (d = 0; d < a.length; d++) b.push(Blockly.Dart.variableDB_.getName(a[d].getId(), Blockly.VARIABLE_CATEGORY_NAME));
...
Blockly.Dart.lists_getIndex = function (a) {
    function b() {
        var a = Blockly.Dart.variableDB_.getDistinctName("tmp_list", Blockly.VARIABLE_CATEGORY_NAME),
            b = "List " + a + " = " + e + ";\n";
        e = a;
        return b
    }
    ...
    if (("RANDOM" != d || "REMOVE" != c) && "FROM_END" != d || e.match(/^\w+$/)) switch (d) {
        ...
        case "RANDOM":
            Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
            if ("REMOVE" == c) return c = Blockly.Dart.variableDB_.getDistinctName("tmp_x", Blockly.VARIABLE_CATEGORY_NAME), "int " + c + " = new Math.Random().nextInt(" + e + ".length);\n" + (e + ".removeAt(" + c + ");\n");
        ...
        if ("RANDOM" == d) return Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;", a = b(), c = Blockly.Dart.variableDB_.getDistinctName("tmp_x", Blockly.VARIABLE_CATEGORY_NAME), a + ("int " + c + " = new Math.Random().nextInt(" + e + ".length);\n") + (e + ".removeAt(" + c + ");\n");
...
Blockly.Dart.lists_setIndex = function (a) {
    ...
    switch (d) {
        ...
        case "RANDOM":
            Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
            a = b();
            d = Blockly.Dart.variableDB_.getDistinctName("tmp_x", Blockly.VARIABLE_CATEGORY_NAME);
...
Blockly.Dart.lists_getSublist = function (a) {
    ...
    } else e = Blockly.Dart.getAdjusted(a, "AT1"), f = Blockly.Dart.getAdjusted(a, "AT2"), a = Blockly.Dart.provideFunction_("lists_get_sublist", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List list, String where1, num at1, String where2, num at2) {",
        "  int getAt(String where, num at) {", "    if (where == 'FROM_END') {", "      at = list.length - 1 - at;", "    } else if (where == 'FIRST') {", "      at = 0;", "    } else if (where == 'LAST') {", "      at = list.length - 1;", "    } else if (where != 'FROM_START') {", "      throw 'Unhandled option (lists_getSublist).';", "    }", "    return at;", "  }", "  at1 = getAt(where1, at1);", "  at2 = getAt(where2, at2) + 1;", "  return list.sublist(at1, at2);", "}"]) + "(" + b + ", '" + c + "', " + e + ", '" + d + "', " + f + ")";
    return [a,
        Blockly.Dart.ORDER_UNARY_POSTFIX]
};
Blockly.Dart.lists_sort = function (a) {
    ...
    return [Blockly.Dart.provideFunction_("lists_sort", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List list, String type, int direction) {", "  var compareFuncs = {", '    "NUMERIC": (a, b) => (direction * a.compareTo(b)).toInt(),', '    "TEXT": (a, b) => direction * a.toString().compareTo(b.toString()),', '    "IGNORE_CASE": ',
...
Blockly.Dart.controls_repeat_ext = function (a) {
    ...
    a = "";
    var d = Blockly.Dart.variableDB_.getDistinctName("count", Blockly.VARIABLE_CATEGORY_NAME), e = b;
    b.match(/^\w+$/) || Blockly.isNumber(b) || (e = Blockly.Dart.variableDB_.getDistinctName("repeat_end", Blockly.VARIABLE_CATEGORY_NAME), a += "var " + e + " = " + b + ";\n");
...
Blockly.Dart.controls_for = function (a) {
    var b = Blockly.Dart.variableDB_.getName(a.getFieldValue("VAR"), Blockly.VARIABLE_CATEGORY_NAME),
    ...
    if (Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e)) {
        ...
        "_inc", Blockly.VARIABLE_CATEGORY_NAME), a += "num " + d + " = ", a = Blockly.isNumber(e) ? a + (Math.abs(e) + ";\n") : a + ("(" + e + ").abs();\n"), a = a + ("if (" + g + " > " + c + ") {\n") + (Blockly.Dart.INDENT + d + " = -" + d + ";\n"), a += "}\n", a += "for (" + b + " = " + g + "; " + d + " >= 0 ? " + b + " <= " + c + " : " + b + " >= " + c + "; " + b + " += " + d + ") {\n" + f + "}\n";
    return a
};
...
Blockly.Dart.math_change = function (a) {
    ...
    a = Blockly.Dart.variableDB_.getName(a.getFieldValue("VAR"), Blockly.VARIABLE_CATEGORY_NAME);
    ...
    switch (b) {
        ...
        case "MIN":
            ...
            b = Blockly.Dart.provideFunction_("math_min",
                ["num " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List<num> myList) {", "  if (myList.isEmpty) return null;", "  num minVal = myList[0];", "  myList.forEach((num entry) {minVal = Math.min(minVal, entry);});", "  return minVal;", "}"]);
            b = b + "(" + a + ")";
            break;
        case "MAX":
            Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
            b = Blockly.Dart.provideFunction_("math_max", ["num " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List<num> myList) {", "  if (myList.isEmpty) return null;", "  num maxVal = myList[0];",
...
Blockly.Dart.procedures_defreturn = function (a) {
    var b = Blockly.Dart.variableDB_.getName(a.getFieldValue("NAME"), Blockly.PROCEDURE_CATEGORY_NAME), c = "";
    Blockly.Dart.STATEMENT_PREFIX && (c += Blockly.Dart.injectId(Blockly.Dart.STATEMENT_PREFIX, a));
    ...
    for (var l = f ? "dynamic" : "void", k = [], h = 0; h < a.arguments_.length; h++) k[h] = Blockly.Dart.variableDB_.getName(a.arguments_[h], Blockly.VARIABLE_CATEGORY_NAME);
    c = l + " " + b + "(" + k.join(", ") + ") {\n" + c + d + e + g + f + "}";
...
Blockly.Dart.procedures_callreturn = function (a) {
    for (var b = Blockly.Dart.variableDB_.getName(a.getFieldValue("NAME"), Blockly.PROCEDURE_CATEGORY_NAME), c = [], d = 0; d < a.arguments_.length; d++) c[d] = Blockly.Dart.valueToCode(a, "ARG" + d, Blockly.Dart.ORDER_NONE) || "null";
...
Blockly.Dart.text_getSubstring = function (a) {
    ...
    } else e = Blockly.Dart.getAdjusted(a, "AT1"), f = Blockly.Dart.getAdjusted(a, "AT2"), a = Blockly.Dart.provideFunction_("text_get_substring", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String text, String where1, num at1, String where2, num at2) {",
...
Blockly.Dart.variables_get = function (a) {
    return [Blockly.Dart.variableDB_.getName(a.getFieldValue("VAR"), Blockly.VARIABLE_CATEGORY_NAME), Blockly.Dart.ORDER_ATOMIC]
};
Blockly.Dart.variables_set = function (a) {
    var b = Blockly.Dart.valueToCode(a, "VALUE", Blockly.Dart.ORDER_ASSIGNMENT) || "0";
    return Blockly.Dart.variableDB_.getName(a.getFieldValue("VAR"), Blockly.VARIABLE_CATEGORY_NAME) + " = " + b + ";\n"
};
