...
def test_get_text_complex():
    ...
    number_of_calls = 0
    assertEquals(text.find(text_random_letter((get_Blockly() if True else None))) + 1 > 0, True,
...
def test_substring_simple():
    ...
    assertEquals(text[int((0 + 1) - 1): -int((0 + 1) - 1) or sys.maxsize], '123456789',
...
def test_substring_complex():
    ...
    number_of_calls = 0
    assertEquals((get_numbers() if True else None)[int((2 if True else None) - 1): int(3 if True else None)], '23',
    ...
    number_of_calls = 0
    assertEquals((get_numbers() if True else None)[-int(0 + 3): -int((0 + 2) - 1) or sys.maxsize], '78',
    ...
    number_of_calls = 0
    # Checks that the whole string is properly retrieved even if the value for start and end is not a simple number. This is especially important in generators where substring uses [x:length - y] for # #-end.
    assertEquals(get_numbers()[int((0 + 1) - 1): -int((0 + 1) - 1) or sys.maxsize], '123456789',
...
def test_get_lists_create_list():
    ...
    assertEquals(first_index(['Kirk', 'Spock', 'McCoy'], random.choice(['Kirk', 'Spock', 'McCoy'])) > 0, True,
                 'get random simple')
...
def test_get_lists_complex():
    ...
    check_number_of_calls('get random complex')
    number_of_calls = 0
    assertEquals(first_index(list2, random.choice(get_star_wars() if True else None)) > 0, True,
                 'get random order complex')
    check_number_of_calls('get random order complex')
    number_of_calls = 0
...
def test_getRemove():
    ...
    list2 = ['Kirk', 'Spock', 'McCoy']
    assertEquals(first_index(list2, lists_remove_random_item(list2 if True else None)) == 0, True,
...
def test_sublist_simple():
    ...
    assertEquals(list2[int((2 if True else None) - 1): int(3 if True else None)], ['Challenger', 'Discovery'],
                 'sublist # simple order')
    assertEquals(list2[-3: -1], ['Discovery', 'Atlantis'], 'sublist #-end simple')
    # The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
    assertEquals(list2[-int(0 + 3): -int((0 + 2) - 1) or sys.maxsize], ['Discovery', 'Atlantis'],
    ...
    assertEquals(list2[int((0 + 1) - 1): -int((0 + 1) - 1) or sys.maxsize], list2,
...
def test_sublist_complex():
    ...
    number_of_calls = 0
    assertEquals((get_space_shuttles() if True else None)[int((2 if True else None) - 1): int(3 if True else None)],
                 ['Challenger', 'Discovery'], 'sublist # start order complex')
    ...
    number_of_calls = 0
    assertEquals((get_space_shuttles() if True else None)[-int(0 + 3): -int((0 + 2) - 1) or sys.maxsize],
                 ['Discovery', 'Atlantis'], 'sublist # end order complex')
    ...
    check_number_of_calls('sublist #-end # complex')
    number_of_calls = 0
    assertEquals(get_space_shuttles()[: 4], ['Columbia', 'Challenger', 'Discovery', 'Atlantis'],
    ...
    number_of_calls = 0
    assertEquals(get_space_shuttles()[-4:], ['Challenger', 'Discovery', 'Atlantis', 'Endeavour'],
    ...
    number_of_calls = 0
    # Checks that the whole list is properly retrieved even if the value for start and end is not a simple number. This is especially important in generators where sublist uses [x:length - y] for # #-end.
    assertEquals(get_space_shuttles()[int((0 + 1) - 1): -int((0 + 1) - 1) or sys.maxsize], list2,
...
def test_sort_alphabetic():
    ...
    assertEquals(lists_sort(list2 if True else None, "TEXT", False), ['Borg', 'Vulcan', 'klingon'],
...
def test_sort_ignoreCase():
    ...
    assertEquals(lists_sort(list2 if True else None, "IGNORE_CASE", False), ['Borg', 'klingon', 'Vulcan'],
...
def test_colour_random():
    ...
    for count4 in range(100):
        ...
        for i in range(1, 7):
            assertEquals(0 != 'abcdefABDEF0123456789'.find(item[int((i + 1) - 1)]) + 1, True, ''.join(
                [str(x4) for x4 in ['contents of random colour string: ', item, ' at index: ', i + 1]]))
